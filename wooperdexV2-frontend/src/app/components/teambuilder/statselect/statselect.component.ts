...
export class StatselectComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.store.currentPokemon$.subscribe((pokemon) => {
      ...
      if (pokemon.nature) {
        ...
        const fullNature = this.natures.find((n) =>
          n.startsWith(pokemon.nature as string)
        );
        if (fullNature) {
  ...
  canIncreaseEv(stat: keyof BaseStats): boolean {
    ...
    return (
      totalEvs + 4 <= this.statsService.evTotalMax &&
  ...
  onEvInput(stat: keyof BaseStats, event: Event): void {
    ...
    if (otherStatsTotalEvs + roundedValue > this.statsService.evTotalMax) {
      finalValue = Math.max(
        0,
        this.statsService.evTotalMax - otherStatsTotalEvs
      );
  ...
  getRemainingEvs(): number {
    const currentEvs = this.evForm.value as BaseStats;
    return (
      this.statsService.evTotalMax -
      Object.values(currentEvs).reduce((a, b) => a + b, 0)
    );
  }
  ...
  onIvInput(stat: keyof BaseStats, event: Event): void {
    ...
    let finalValue = Math.max(
      this.statsService.ivMin,
      Math.min(this.statsService.ivMax, inputValue)
    );
  ...
  calculateFinalStat(stat: keyof BaseStats): number {
    ...
    return this.statsService.calculateFinalStat(
      baseValue,
      ev,
      iv,
      natureEffect,
